...
/**
 * AdditionGame - An interactive arithmetic game for practicing addition.
 ...
 */
// PUBLIC_INTERFACE
function AdditionGame({ onClose }) {
  // Generate two new random numbers for addition
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const correct = Number(userInput) === problem.a + problem.b;
    setSuccess(correct);
    if (correct) {
      setFeedback("✅ Correct!");
      setStreak((s) => s + 1);
  ...
  useEffect(() => {
    const handler = (e) => {
      if (e.key === "Enter") {
        document.getElementById("addition-submit-btn")?.click();
      }
    };
  ...
  return (
    ...
    >
      <button
        aria-label="Close Addition Game"
        onClick={onClose}
        style={{
          position: "absolute",
          top: 15,
      ...
      <div className="calc-display" style={{ textAlign: "center", fontSize: "1.4rem", marginBottom: 25 }}>
        {problem.a} <span style={{ color: "var(--primary)", fontWeight: 700, fontSize: "1.6rem" }}>+</span> {problem.b} = ?
      </div>
      ...
      >
        <span style={{ color: "var(--accent)", fontWeight: 600 }}>Addition Practice</span>
        &nbsp;•&nbsp; Modern UI
      </footer>
    </div>
  );
